...
> 发布前待补：本人三关试玩记录、PSP 实际耗时；确认 GitHub 上传成功，并上传本页图片至博客园。本文开发和测试部分基于本次实际执行结果，尚未将这些个人待办记为已完成。
...
- 发布阶段发现 GitHub 连接账号与目标仓库所属账号不同，当前连接没有写权限。代码和多次提交先保存在本地，不能把这种情况写成已上传。
